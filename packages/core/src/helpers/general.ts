...
export const applyOutputToMessages = (warp: Warp, output: Record<string, any>): Record<string, string> => {
  const parts = Object.entries(warp.messages || {}).map(([key, value]) => [key, replacePlaceholders(value, output)])
