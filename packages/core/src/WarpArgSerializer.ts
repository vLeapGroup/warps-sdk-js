import {
  ...
  export class WarpArgSerializer {
    ...
    typedToString(value: TypedValue): string {
      ...
      if (value.hasClassOrSuperclass(Struct.ClassName)) {
        const structFields = (value as Struct).getFields();
        const result: Record<string, any> = {};
        for (const field of structFields) {
          const fieldString = this.typedToString(field.value);
          const colonIndex = fieldString.indexOf(":");
          result[field.name] = colonIndex !== -1 ? fieldString.slice(colonIndex + 1) : fieldString;
        }
